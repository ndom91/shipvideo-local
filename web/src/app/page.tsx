...
const EXAMPLES: Record<Mode, string> = {
  ...
  prompt:
    "A direct launch video for a developer tool that makes LLM inference fast and predictable.",
};
...
export default function Home() {
  ...
  const selected =
    history.find((item) => item.jobId === selectedId) ?? history[0];
    ...
    <main id="content" tabIndex={-1} className="flex-1 px-5 pb-12 sm:px-8">
      <div className="mx-auto w-full max-w-6xl">
        ...
        <header className="border-b border-line py-8 sm:flex sm:items-end sm:justify-between">
          ...
          <p className="mt-3 max-w-sm text-sm leading-6 text-muted sm:mt-0">
            Claude Code writes the scene. This Mac renders the MP4. Previous
            work stays in your local library.
          </p>
        </header>

        <section
          className="grid gap-8 py-8 lg:grid-cols-[minmax(0,1fr)_20rem]"
          aria-labelledby="new-render-heading"
        >
          <div>
            <h2 id="new-render-heading" className="sr-only">
              New render
            </h2>
            <form
              onSubmit={(event) => {
                event.preventDefault();
                if (!submitting && input.trim()) void submit();
              }}
              className="border border-line bg-white/[0.02] p-4 sm:p-5"
            >
                ...
                <div className="flex gap-1">
                  {(["url", "prompt"] as Mode[]).map((item) => (
                    <button
                      ...
                      className={`rounded-md px-3 py-1.5 text-sm font-medium transition-colors ${mode === item ? "bg-foreground text-background" : "text-muted hover:bg-white/[0.06] hover:text-foreground"}`}
                    >
              ...
              <label
                htmlFor="render-input"
                className="mt-5 block text-sm font-medium"
              >
                {mode === "url"
                  ? "Website to study"
                  : "What should this video explain?"}
              </label>
              {mode === "url" ? (
                <input
                  id="render-input"
                  ...
                  inputMode="url"
                  className="mt-2 w-full border-b border-line bg-transparent py-3 text-lg outline-none placeholder:text-muted/60 focus:border-accent"
                />
              ) : (
                <textarea
                  id="render-input"
                  ...
                  value={input}
                  onChange={(event) => setInput(event.target.value)}
                  placeholder="Describe the product, audience, and the point you want to make."
                  rows={4}
                  className="mt-2 w-full resize-none border-b border-line bg-transparent py-3 text-lg outline-none placeholder:text-muted/60 focus:border-accent"
                />
              )}
              <div className="mt-5 flex items-center justify-between gap-4">
                <button
                  ...
                  className="text-sm text-muted underline decoration-line underline-offset-4 hover:text-foreground"
                >
                  Use an example
                </button>
                <button
                  ...
                  className="rounded-md bg-accent px-4 py-2 text-sm font-semibold text-white transition hover:brightness-110 disabled:opacity-40"
                >
            ...
            {error && (
              <p
                role="alert"
                className="mt-3 border-l-2 border-red-400 pl-3 text-sm text-red-300"
              >
                {error}
              </p>
            )}
            ...
            {active && (
              <section
                className="mt-5 border border-line bg-white/[0.015] p-5"
                aria-live="polite"
              >
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="font-medium">
                    {poll?.phase ?? "Starting local Claude Code"}
                  </h2>
                ...
                <p className="mt-5 text-xs text-muted">
                  You can keep working or close this tab. The render will remain
                  in the library when it is complete.
                </p>
              </section>
            )}
            ...
            {job && poll?.status === "error" && (
              <section className="mt-5 border-l-2 border-red-400 bg-red-400/5 p-4">
                ...
                <p className="mt-1 text-sm text-muted">
                  {poll.message ?? "Check the local job log for details."}
                </p>
              </section>
            )}
          </div>
          ...
          <aside
            className="border-t border-line pt-5 lg:border-l lg:border-t-0 lg:pl-7 lg:pt-0"
            aria-labelledby="library-heading"
          >
            <div className="flex items-baseline justify-between">
              <h2 id="library-heading" className="font-medium">
                Local library
              </h2>
              <span className="font-mono text-[11px] text-muted">
                ...
                {history.map((item) => (
                  <li key={item.jobId}>
                    <button
                      ...
                      className={`w-full py-3 text-left ${selected?.jobId === item.jobId ? "text-foreground" : "text-muted hover:text-foreground"}`}
                    >
                      <span className="flex items-center justify-between gap-3">
                        ...
                        <span
                          className={`shrink-0 font-mono text-[10px] uppercase tracking-wide ${item.status === "done" ? "text-foreground" : item.status === "error" ? "text-red-300" : "text-accent"}`}
                        >
                          {item.status}
                        </span>
                      </span>
                      <span className="mt-1 block truncate font-mono text-[11px] text-muted">
                        {relativeTime(item.updatedAt)} · {item.mode}
                      </span>
                    </button>
                  </li>
                ))}
        ...
        {selected?.status === "done" && selected.videoUrl && !activeDone && (
          <section
            ...
            aria-labelledby="selected-render-heading"
          >
            ...
            <video
              ...
              controls
              playsInline
              className="aspect-video w-full max-w-4xl border border-line bg-black"
            />
          </section>
        )}

        <footer className="border-t border-line py-6 font-mono text-[11px] text-muted">
          Local Claude Code · Playwright · ffmpeg
        </footer>
      </div>
    </main>
  );
